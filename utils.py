...
def arg_parse():
    ...
    parser.add_argument('--cuda', action = 'store_true', help = 'Device to train on')
...
def visualize(input, output, gtruth, index = 0, save_dir = None):
    ...
    plt.title("Output")
    plt.subplot(1, 3, 3)
    plt.imshow(gtruth[index], cmap = "gray")
    plt.title("Ground Truth")
    plt.show()
    if save_dir is not None:
        plt.savefig(os.path.join(save_dir, f"{index}.png"))